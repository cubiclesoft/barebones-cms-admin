...
(function() {
  var AttributeUI, ContentTools, CropMarksUI, CropRatioItemUI, EmbedTemplateFieldUI, FindResultItemUI, FindResultTitleUI, StyleUI, exports, _EditorApp,
    __hasProp = {}.hasOwnProperty,
    __extends = function(child, parent) { for (var key in parent) { if (__hasProp.call(parent, key)) child[key] = parent[key]; } function ctor() { this.constructor = child; } ctor.prototype = parent.prototype; child.prototype = new ctor(); child.__super__ = parent.prototype; return child; },
    __bind = function(fn, me){ return function(){ return fn.apply(me, arguments); }; },
    __slice = [].slice;
  ...
  ContentTools.InspectorUI = (function(_super) {
    ...
    InspectorUI.prototype.mount = function() {
      ...
      this.parent().domElement().appendChild(this._domElement);
      domInspectorGroup = this.constructor.createDiv(['ct-inspector__group']);
      this._domElement.appendChild(domInspectorGroup);
      this._domTags = this.constructor.createDiv(['ct-inspector__tags', 'ct-tags']);
      domInspectorGroup.appendChild(this._domTags);
      this._domCounter = this.constructor.createDiv(['ct-inspector__counter']);
      domInspectorGroup.appendChild(this._domCounter);
      this.updateCounter();
      this._addDOMEventListeners();
    ...
    InspectorUI.prototype.unmount = function() {
      ...
      ContentEdit.Root.get().unbind('mount', this._handleFocusChange);
      return ContentEdit.removeCSSClass(document.documentElement, 'ct-inspector--active');
    };

    InspectorUI.prototype.updateCounter = function() {
      var column, completeText, display, element, line, lines, region, sub, word_count, _i, _len, _ref;
      if (!this.isMounted()) {
        return;
      }
      completeText = '';
      ...
      if (!(element && element.type() === 'PreText' && element.selection().isCollapsed())) {
        if (this._domCounter.textContent !== word_count) {
      ...
      if (this._domCounter.textContent !== display) {
        return this._domCounter.textContent = display;
      }
    };
  ...
  ContentTools.TagUI = (function(_super) {
    ...
    function TagUI(element) {
      ...
      this._inlineMoveSize = __bind(this._inlineMoveSize, this);
      this._clearIgnoreTimer = __bind(this._clearIgnoreTimer, this);
      TagUI.__super__.constructor.call(this);
      this._clickTimer = null;
      this._clickIgnoreTimer = null;
    }
    ...
    TagUI.prototype._addDOMEventListeners = function() {
      ...
      this._domElement.addEventListener('touchstart', this._onMouseDown);
      return this._domElement.addEventListener('click', this._onClick);
    };

    TagUI.prototype._clearIgnoreTimer = function() {
      return this._clickIgnoreTimer = null;
    };

    TagUI.prototype._inlineMoveSize = function() {
      var allowScrolling, app, dialog, modal, transparent;
      this._clickTimer = null;
      if (!this.element.can('drag')) {
        return;
      }
      this._clickIgnoreTimer = setTimeout((function(_this) {
        return function() {
          return _this._clearIgnoreTimer();
        };
      })(this), 750);
      app = ContentTools.EditorApp.get();
      if (!app.dispatchEvent(app.createEvent('move-size', {
      ...
      dialog = new ContentTools.MoveSizeDialog(this.element);
      app.attach(modal);
      app.attach(dialog);
      modal.show();
      return dialog.show();
    ...
    TagUI.prototype._onMouseDown = function(ev) {
      if (this.element.typeName() === 'Element') {
        return this._clickTimer = setTimeout((function(_this) {
          return function() {
            return _this._clearIgnoreTimer();
          };
        })(this), ContentEdit.DRAG_HOLD_DURATION);
      } else if (!this._clickTimer && !this._clickIgnoreTimer) {
        return this._clickTimer = setTimeout((function(_this) {
          return function() {
            return _this._inlineMoveSize();
          };
        })(this), ContentEdit.DRAG_HOLD_DURATION);
      } else if (!this._clickIgnoreTimer) {
        return ev.preventDefault();
      }
    };

    TagUI.prototype._onClick = function(ev) {
  ...
  ContentTools.ToolboxUI = (function(_super) {
    ...
    ToolboxUI.prototype.unmount = function() {
      ...
      return ContentEdit.removeCSSClass(document.documentElement, 'ct-toolbox--active');
    };
    ...
    ToolboxUI.prototype._addDOMEventListeners = function() {
      ...
      this._handleKeyDown = (function(_this) {
        return function(ev) {
          var Paragraph, app, element, next, nodename, os, previous, redo, selection, target, undo, version;
          app = ContentTools.EditorApp.get();
          if (app.modals()) {
            return;
          }
          ...
          if (nodename === 'input' || nodename === 'textarea' || nodename === 'select' || nodename === 'button') {
            return;
          }
          element = ContentEdit.Root.get().focused();
          if (element && !element.content) {
            ...
            if (element.navigate) {
              if (ev.keyCode === 37 || ev.keyCode === 38 || (ev.keyCode === 9 && ev.shiftKey)) {
                ev.preventDefault();
                ...
                if (previous) {
                  ...
                  ContentEdit.Root.get().trigger('previous-region', element.closest(function(node) {
                    return node.type() === 'Fixture' || node.type() === 'Region';
                  }));
                }
              }
              if (ev.keyCode === 39 || ev.keyCode === 40 || (ev.keyCode === 9 && !ev.shiftKey)) {
                ev.preventDefault();
                ...
                if (next) {
                  ...
                  ContentEdit.Root.get().trigger('next-region', element.closest(function(node) {
                    return node.type() === 'Fixture' || node.type() === 'Region';
                  }));
                }
              }
          ...
          redo = false;
          undo = false;
          if (os === 'linux' && !ev.altKey && ev.keyCode === 90 && ev.ctrlKey) {
            redo = ev.shiftKey;
            undo = !redo;
          } else if (os === 'mac' && !ev.altKey && !ev.ctrlKey && ev.keyCode === 90 && ev.metaKey) {
            redo = ev.shiftKey;
            undo = !redo;
          } else if (os === 'windows' && !ev.altKey && !ev.shiftKey && ev.ctrlKey) {
            if (ev.keyCode === 89) {
              redo = true;
            }
            if (ev.keyCode === 90) {
  ...
  ContentTools.AnchoredDialogUI = (function(_super) {
    ...
    AnchoredDialogUI.prototype.mount = function() {
      ...
      this.parent().domElement().appendChild(this._domElement);
      this._domElementInner = this.constructor.createDiv(['ct-anchored-dialog-inner']);
      this._domElement.appendChild(this._domElementInner);
      this._contain();
  ...
  ContentTools.DialogUI = (function(_super) {
    ...
    DialogUI.prototype.mount = function() {
      var dialogCSSClasses, dialogWrapperCSSClasses, domBody, domControlsWrap, domHeader, _domElementInner;
      if (document.activeElement) {
        document.activeElement.blur();
        window.getSelection().removeAllRanges();
      }
      dialogWrapperCSSClasses = ['ct-widget', 'ct-dialog-wrapper'];
      this._domElement = this.constructor.createDiv(dialogWrapperCSSClasses);
      this.parent().domElement().appendChild(this._domElement);
      dialogCSSClasses = ['ct-dialog'];
      if (this._busy) {
        dialogCSSClasses.push('ct-dialog--busy');
      }
      _domElementInner = this.constructor.createDiv(dialogCSSClasses);
      this._domElement.appendChild(_domElementInner);
      ...
      domControlsWrap = this.constructor.createDiv(['ct-dialog__controls-wrap']);
      domBody.appendChild(domControlsWrap);
      this._domControls = this.constructor.createDiv(['ct-dialog__controls']);
      domControlsWrap.appendChild(this._domControls);
      this._domBusy = this.constructor.createDiv(['ct-dialog__busy']);
      return _domElementInner.appendChild(this._domBusy);
    };
    ...
    DialogUI.prototype._addDOMEventListeners = function() {
      ...
      this._domClose.addEventListener('click', (function(_this) {
        return function(ev) {
          ev.preventDefault();
          if (_this._busy) {
            return;
      ...
      return this._domElement.addEventListener('keydown', (function(_this) {
        return function(ev) {
          if (ev.keyCode !== 27) {
  ...
  ContentTools.LinkDialog = (function(_super) {
    ...
    LinkDialog.prototype.mount = function() {
      ...
      this._domElementInner.appendChild(this._domInput);
      this._domTargetButton = this.constructor.createDiv(['ct-anchored-dialog__target-button']);
      this._domElementInner.appendChild(this._domTargetButton);
      if (this._target === NEW_WINDOW_TARGET) {
        ContentEdit.addCSSClass(this._domTargetButton, 'ct-anchored-dialog__target-button--active');
      }
      this._domButton = this.constructor.createDiv(['ct-anchored-dialog__button', 'ct-anchored-dialog__confirm-button']);
      this._domElementInner.appendChild(this._domButton);
      return this._addDOMEventListeners();
    };
  ...
  ContentTools.MoveSizeDialog = (function(_super) {
    __extends(MoveSizeDialog, _super);

    function MoveSizeDialog(element) {
      MoveSizeDialog.__super__.constructor.call(this);
      this._moveSizeElement = element;
      this.position(this._getCurrentPosition());
    }

    MoveSizeDialog.prototype.mount = function() {
      var size;
      MoveSizeDialog.__super__.mount.call(this);
      if (this._moveSizeElement instanceof ContentEdit.ResizableElement && this._moveSizeElement.can('resize') && ContentEdit.RESIZE_CORNER_SIZE) {
        size = this._moveSizeElement.size();
        ContentEdit.addCSSClass(this._domElement, 'ct-anchored-dialog__move-size-buttons');
      } else {
        ContentEdit.addCSSClass(this._domElement, 'ct-anchored-dialog__move-buttons');
      }
      this.position(this._getCurrentPosition());
      this._domMoveUpLeftButton = this.constructor.createDiv(['ct-anchored-dialog__button', 'ct-anchored-dialog__move-up-left-button']);
      this._domElementInner.appendChild(this._domMoveUpLeftButton);
      this._domMoveUpButton = this.constructor.createDiv(['ct-anchored-dialog__button', 'ct-anchored-dialog__move-up-button']);
      this._domElementInner.appendChild(this._domMoveUpButton);
      this._domMoveUpRightButton = this.constructor.createDiv(['ct-anchored-dialog__button', 'ct-anchored-dialog__move-up-right-button']);
      this._domElementInner.appendChild(this._domMoveUpRightButton);
      if (this._moveSizeElement instanceof ContentEdit.ResizableElement && this._moveSizeElement.can('resize') && ContentEdit.RESIZE_CORNER_SIZE) {
        this._domWidthInput = document.createElement('input');
        this._domWidthInput.setAttribute('class', 'ct-anchored-dialog__size-input ct-anchored-dialog__size-input-first');
        this._domWidthInput.setAttribute('name', 'width');
        this._domWidthInput.setAttribute('placeholder', ContentEdit._('Enter a width') + '...');
        this._domWidthInput.setAttribute('type', 'text');
        this._domWidthInput.setAttribute('value', size[0]);
        this._domElementInner.appendChild(this._domWidthInput);
      }
      this._domMoveDownLeftButton = this.constructor.createDiv(['ct-anchored-dialog__button', 'ct-anchored-dialog__move-down-left-button']);
      this._domElementInner.appendChild(this._domMoveDownLeftButton);
      this._domMoveDownButton = this.constructor.createDiv(['ct-anchored-dialog__button', 'ct-anchored-dialog__move-down-button']);
      this._domElementInner.appendChild(this._domMoveDownButton);
      this._domMoveDownRightButton = this.constructor.createDiv(['ct-anchored-dialog__button', 'ct-anchored-dialog__move-down-right-button']);
      this._domElementInner.appendChild(this._domMoveDownRightButton);
      if (this._moveSizeElement instanceof ContentEdit.ResizableElement && this._moveSizeElement.can('resize') && ContentEdit.RESIZE_CORNER_SIZE) {
        this._domHeightInput = document.createElement('input');
        this._domHeightInput.setAttribute('class', 'ct-anchored-dialog__size-input');
        this._domHeightInput.setAttribute('name', 'height');
        this._domHeightInput.setAttribute('placeholder', ContentEdit._('Enter a height') + '...');
        this._domHeightInput.setAttribute('type', 'text');
        this._domHeightInput.setAttribute('value', size[1]);
        this._domElementInner.appendChild(this._domHeightInput);
      }
      return this._addDOMEventListeners();
    };

    MoveSizeDialog.prototype._getCurrentPosition = function() {
      var currPosition, rect, scrollX, scrollY, _ref;
      _ref = ContentTools.getScrollPosition(), scrollX = _ref[0], scrollY = _ref[1];
      rect = this._moveSizeElement.domElement().getBoundingClientRect();
      currPosition = [rect.left + (rect.width / 2) + scrollX, rect.top + (rect.height / 2) + scrollY];
      return currPosition;
    };

    MoveSizeDialog.prototype._adjustPosition = function(origPosition) {
      var currElement, newPosition, scrollElement;
      newPosition = this._getCurrentPosition();
      this.position(newPosition);
      if (ContentTools.MOVE_SIZE_AUTO_SCROLL) {
        scrollElement = null;
        currElement = this._moveSizeElement.domElement().parentNode;
        while (currElement && !scrollElement) {
          if (currElement.clientHeight > 0 && currElement.scrollHeight - 25 > currElement.clientHeight) {
        ...
        return scrollElement.scrollTop += newPosition[1] - origPosition[1];
      }
    };

    MoveSizeDialog.prototype._move = function(vert, horz) {
      var currElement, newElement, origPosition;
      newElement = null;
      currElement = this._moveSizeElement;
      if (vert === 'up') {
        currElement = currElement.previous();
        while (currElement && !newElement) {
          if (currElement !== this._moveSizeElement.parent() && currElement.can('drop') && (currElement.constructor.droppers[this._moveSizeElement.type()] || this._moveSizeElement.constructor.droppers[currElement.type()])) {
      ...
      origPosition = this._getCurrentPosition();
      if (vert === 'up') {
        if (newElement === this._moveSizeElement.previousSibling()) {
          this._moveSizeElement.drop(newElement, ['above', horz]);
        } else {
          this._moveSizeElement.drop(newElement, ['below', horz]);
        }
      } else {
        if (newElement === this._moveSizeElement.nextSibling()) {
          this._moveSizeElement.drop(newElement, ['below', horz]);
        } else {
          this._moveSizeElement.drop(newElement, ['above', horz]);
        }
      }
      return this._adjustPosition(origPosition);
    };

    MoveSizeDialog.prototype._widthUpdated = function() {
      var origPosition;
      if (this._domWidthInput.value === '') {
        return;
      }
      this._domHeightInput.value = Math.round(parseInt(this._domWidthInput.value) * this._moveSizeElement.aspectRatio());
      origPosition = this._getCurrentPosition();
      this._moveSizeElement.size([this._domWidthInput.value, this._domHeightInput.value]);
      return this._adjustPosition(origPosition);
    };

    MoveSizeDialog.prototype._heightUpdated = function() {
      var origPosition;
      if (this._domHeightInput.value === '') {
        return;
      }
      this._domWidthInput.value = Math.round(parseInt(this._domHeightInput.value) / this._moveSizeElement.aspectRatio());
      origPosition = this._getCurrentPosition();
      this._moveSizeElement.size([this._domWidthInput.value, this._domHeightInput.value]);
      return this._adjustPosition(origPosition);
    };

    MoveSizeDialog.prototype._addDOMEventListeners = function() {
      this._domMoveUpLeftButton.addEventListener('click', (function(_this) {
        return function(ev) {
          ev.preventDefault();
          return _this._move('up', 'left');
        };
      })(this));
      this._domMoveUpButton.addEventListener('click', (function(_this) {
        return function(ev) {
          ev.preventDefault();
          return _this._move('up', 'center');
        };
      })(this));
      this._domMoveUpRightButton.addEventListener('click', (function(_this) {
        return function(ev) {
          ev.preventDefault();
          return _this._move('up', 'right');
        };
      })(this));
      this._domMoveDownLeftButton.addEventListener('click', (function(_this) {
        return function(ev) {
          ev.preventDefault();
          return _this._move('down', 'left');
        };
      })(this));
      this._domMoveDownButton.addEventListener('click', (function(_this) {
        return function(ev) {
          ev.preventDefault();
          return _this._move('down', 'center');
        };
      })(this));
      this._domMoveDownRightButton.addEventListener('click', (function(_this) {
        return function(ev) {
          ev.preventDefault();
          return _this._move('down', 'right');
        };
      })(this));
      if (this._moveSizeElement instanceof ContentEdit.ResizableElement && this._moveSizeElement.can('resize') && ContentEdit.RESIZE_CORNER_SIZE) {
        this._domWidthInput.addEventListener('keypress', (function(_this) {
          return function(ev) {
            return _this._widthUpdated();
          };
        })(this));
        this._domWidthInput.addEventListener('keyup', (function(_this) {
          return function(ev) {
            return _this._widthUpdated();
          };
        })(this));
        this._domWidthInput.addEventListener('input', (function(_this) {
          return function(ev) {
            return _this._widthUpdated();
          };
        })(this));
        this._domWidthInput.addEventListener('change', (function(_this) {
          return function(ev) {
            return _this._widthUpdated();
          };
        })(this));
        this._domWidthInput.addEventListener('blur', (function(_this) {
          return function(ev) {
            var size;
            size = _this._moveSizeElement.size();
            _this._domWidthInput.value = size[0];
            return _this._domHeightInput.value = size[1];
          };
        })(this));
        this._domHeightInput.addEventListener('keypress', (function(_this) {
          return function(ev) {
            return _this._heightUpdated();
          };
        })(this));
        this._domHeightInput.addEventListener('keyup', (function(_this) {
          return function(ev) {
            return _this._heightUpdated();
          };
        })(this));
        this._domHeightInput.addEventListener('input', (function(_this) {
          return function(ev) {
            return _this._heightUpdated();
          };
        })(this));
        this._domHeightInput.addEventListener('change', (function(_this) {
          return function(ev) {
            return _this._heightUpdated();
          };
        })(this));
        return this._domHeightInput.addEventListener('blur', (function(_this) {
          return function(ev) {
            var size;
            size = _this._moveSizeElement.size();
            _this._domWidthInput.value = size[0];
  ...
  StyleUI = (function(_super) {
    ...
    StyleUI.prototype.mount = function(domParent, before) {
      ...
      switchWrap = this.constructor.createDiv(['ct-section__switch-wrap']);
      switchWrap.appendChild(this.constructor.createDiv(['ct-section__switch']));
      this._domElement.appendChild(switchWrap);
  ...
  ContentTools.TableDialog = (function(_super) {
    ...
    TableDialog.prototype.mount = function() {
      var cfg, domBodyInputWrap, domBodyLabel, domControlGroup, domFootLabel, domHeadLabel, footCSSClasses, headCSSClasses, sectionWrap, switchWrap;
      ...
      sectionWrap = this.constructor.createDiv(['ct-section-wrap']);
      this._domView.appendChild(sectionWrap);
      ...
      sectionWrap.appendChild(this._domHeadSection);
      ...
      switchWrap.appendChild(this._domHeadSwitch);
      this._domHeadSection.appendChild(switchWrap);
      this._domBodySection = this.constructor.createDiv(['ct-section', 'ct-section--applied', 'ct-section--contains-input']);
      sectionWrap.appendChild(this._domBodySection);
      ...
      domBodyInputWrap = this.constructor.createDiv(['ct-section__input-wrap']);
      this._domBodySection.appendChild(domBodyInputWrap);
      ...
      domBodyInputWrap.appendChild(this._domBodyInput);
      ...
      sectionWrap.appendChild(this._domFootSection);
      ...
      switchWrap.appendChild(this._domFootSwitch);
      this._domFootSection.appendChild(switchWrap);
  ...
  ContentTools.VideoDialog = (function(_super) {
    ...
    VideoDialog.prototype.mount = function() {
      ...
      domControlGroup = this.constructor.createDiv(['ct-control-group', 'ct-control-group--left']);
      this._domControls.appendChild(domControlGroup);
      this._domInput = document.createElement('input');
  ...
  _EditorApp = (function(_super) {
    ...
    _EditorApp.prototype.modals = function() {
      return this._modals;
    };
    ...
    _EditorApp.prototype.addedModal = function() {
      return this._modals++;
    };

    _EditorApp.prototype.removedModal = function() {
      return this._modals--;
    };
  ...
  ContentTools.Tools.Address = (function(_super) {
    ...
    ContentTools.ToolShelf.stow(Address, 'address');
  ...
  ContentTools.EmbedTemplateDialog = (function(_super) {
    __extends(EmbedTemplateDialog, _super);

    function EmbedTemplateDialog() {
      EmbedTemplateDialog.__super__.constructor.call(this, 'Insert Template');
      this._currSelection = '';
      this._fieldSets = {
        '': []
      };
    ...
    EmbedTemplateDialog.prototype.mount = function() {
      var domControlGroup, field, fieldSet, newfield, num, option, template, _i, _j, _len, _len1, _ref, _ref1;
      EmbedTemplateDialog.__super__.mount.call(this);
      ContentEdit.addCSSClass(this._domElement, 'ct-embed-template-dialog');
      ContentEdit.addCSSClass(this._domView, 'ct-embed-template-dialog__view');
      this._domTemplate = document.createElement('select');
      this._domTemplate.setAttribute('class', 'ct-embed-template-dialog__select');
      this._domTemplate.setAttribute('name', 'template');
      option = document.createElement('option');
      option.value = '';
      option.textContent = ContentEdit._('Select a template');
      this._domTemplate.appendChild(option);
      this._domView.appendChild(this._domTemplate);
      _ref = ContentTools.EMBED_TEMPLATES;
    ...
    EmbedTemplateDialog.prototype.save = function() {
      return this.dispatchEvent(this.createEvent('save', {
        'content': this._content
      }));
    };

    EmbedTemplateDialog.prototype.show = function() {
      EmbedTemplateDialog.__super__.show.call(this);
      return this._domTemplate.focus();
    };

    EmbedTemplateDialog.prototype.unmount = function() {
      if (this.isMounted()) {
        this._domTemplate.blur();
      }
      EmbedTemplateDialog.__super__.unmount.call(this);
      this._domButton = null;
      return this._domTemplate = null;
    };

    EmbedTemplateDialog.prototype.updateContent = function() {
      var field, fieldMap, fieldui, valid, _i, _len, _ref;
      this._content = '';
      valid = true;
      fieldMap = {};
      _ref = this._fieldSets[this._currSelection];
      for (_i = 0, _len = _ref.length; _i < _len; _i++) {
        fieldui = _ref[_i];
        if (!fieldui.valid()) {
          valid = false;
      ...
      if (valid && this._currSelection) {
        this._content = ContentTools.EMBED_TEMPLATES[parseInt(this._currSelection, 10)].content(fieldMap).trim();
      }
      if (this._content) {
    ...
    EmbedTemplateDialog.prototype._changeTemplate = function() {
      ...
      this._currSelection = this._domTemplate.value;
      _ref1 = this._fieldSets[this._currSelection];
      for (_j = 0, _len1 = _ref1.length; _j < _len1; _j++) {
        field = _ref1[_j];
        field.mount(this._domView);
      }
      return this.updateContent();
    };

    EmbedTemplateDialog.prototype._addDOMEventListeners = function() {
      EmbedTemplateDialog.__super__._addDOMEventListeners.call(this);
      this._domTemplate.addEventListener('change', (function(_this) {
        return function(ev) {
          return _this._changeTemplate();
        };
      })(this));
      this._domTemplate.addEventListener('keydown', (function(_this) {
        return function(ev) {
          return _this._changeTemplate();
        };
      })(this));
    ...
    return EmbedTemplateDialog;

  })(ContentTools.DialogUI);

  EmbedTemplateFieldUI = (function(_super) {
    __extends(EmbedTemplateFieldUI, _super);

    function EmbedTemplateFieldUI(parentDialog, field) {
      this.parentDialog = parentDialog;
      this.field = field;
      EmbedTemplateFieldUI.__super__.constructor.call(this);
      this.field.value = this.field["default"];
    }

    EmbedTemplateFieldUI.prototype.valid = function() {
      if ((!(this.field.required != null) || (this.field.required && this.field.value)) && (!(this.field.valid != null) || this.field.valid(this.field.value))) {
        ContentEdit.removeCSSClass(this._domLabel, 'ct-section__input--invalid');
        return true;
      } else {
        ContentEdit.addCSSClass(this._domLabel, 'ct-section__input--invalid');
        return false;
      }
    };

    EmbedTemplateFieldUI.prototype.getField = function() {
      return this.field;
    };

    EmbedTemplateFieldUI.prototype.mount = function(domParent, before) {
      var button, key, option, val, _ref;
      if (before == null) {
        before = null;
      }
      this._domElement = this.constructor.createDiv(['ct-section-wrap']);
      this._section = this.constructor.createDiv(['ct-section', 'ct-section--applied']);
      this._domElement.appendChild(this._section);
      this._domLabel = this.constructor.createDiv(['ct-section__label']);
      this._domLabel.textContent = this.field.title;
      this._section.appendChild(this._domLabel);
      if (this.field.type === 'text') {
        this._domInputWrap = this.constructor.createDiv(['ct-section__input-wrap']);
        this._domInput = document.createElement('input');
        this._domInput.setAttribute('class', 'ct-section__input');
        this._domInput.setAttribute('type', 'text');
        this._domInput.setAttribute('value', this.field.value);
        this._domInputWrap.appendChild(this._domInput);
        this._section.appendChild(this._domInputWrap);
      } else if (this.field.type === 'switch') {
        this._domInputWrap = this.constructor.createDiv(['ct-section__switch-wrap']);
        this._domInput = this.constructor.createDiv(['ct-section__switch']);
        this._domInputWrap.appendChild(this._domInput);
        this._section.appendChild(this._domInputWrap);
        if (!this.field.value) {
          ContentEdit.removeCSSClass(this._section, 'ct-section--applied');
        }
        ...
        this._domInputWrap.appendChild(this._domInput);
        this._section.appendChild(this._domInputWrap);
        _ref = this.field.options;
        for (key in _ref) {
          val = _ref[key];
          option = document.createElement('option');
          option.value = key;
          option.textContent = val;
          if (key === this.field.value) {
            option.setAttribute("selected", "true");
          }
          this._domInput.appendChild(option);
        }
      } else if (this.field.type === 'textarea') {
        this._domInputWrap = this.constructor.createDiv(['ct-section__input-wrap', 'ct-section__textarea-wrap']);
        this._domInput = document.createElement('textarea');
        this._domInput.setAttribute('class', 'ct-section__input ct-section__textarea');
        this._domInput.textContent = this.field.value;
        this._domInputWrap.appendChild(this._domInput);
        this._section.appendChild(this._domInputWrap);
      } else if (this.field.type === 'file') {
        this._domInputWrap = this.constructor.createDiv(['ct-section__input-wrap', 'ct-section__file-wrap']);
        this._domInput = this.constructor.createDiv(['ct-section__input']);
        button = this.constructor.createDiv(['ct-control--text']);
        button.textContent = ContentEdit._('Select file...');
        this._domInput.appendChild(button);
        this._domInputWrap.appendChild(this._domInput);
        this._section.appendChild(this._domInputWrap);
      }
      return EmbedTemplateFieldUI.__super__.mount.call(this, domParent, before);
    };

    EmbedTemplateFieldUI.prototype._addDOMEventListeners = function() {
      if (this.field.type === 'text' || this.field.type === 'textarea') {
        this._section.addEventListener('click', (function(_this) {
          return function(ev) {
            return _this._domInput.focus();
          };
        })(this));
        return this._domInput.addEventListener('input', (function(_this) {
          return function(ev) {
            _this.field.value = _this._domInput.value;
            return _this.parentDialog.updateContent();
          };
        })(this));
      } else if (this.field.type === 'select') {
        this._section.addEventListener('click', (function(_this) {
        ...
        return this._section.addEventListener('click', (function(_this) {
          return function(ev) {
            ev.preventDefault();
            _this.field.value = !_this.field.value;
            if (_this.field.value) {
              ContentEdit.addCSSClass(_this._section, 'ct-section--applied');
            } else {
              ContentEdit.removeCSSClass(_this._section, 'ct-section--applied');
            }
            return _this.parentDialog.updateContent();
          };
        })(this));
        ...
        this._section.addEventListener('click', (function(_this) {
          return function(ev) {
            ev.preventDefault();
            return _this._domInput.click();
          };
        })(this));
        return this._domInput.addEventListener('click', (function(_this) {
          return function(ev) {
            var app, dialog;
            ev.preventDefault();
            ev.stopPropagation();
            ...
            dialog.addEventListener('cancel', function() {
              dialog.hide();
              return _this.parentDialog.busy(false);
            });
            dialog.addEventListener('save', function(ev) {
              ...
              for (_i = 0, _len = selected.length; _i < _len; _i++) {
                item = selected[_i];
                if (item.info) {
                  _this.field.value = item;
                }
              }
              dialog.hide();
              _this.parentDialog.busy(false);
              return _this.parentDialog.updateContent();
            });
            app.attach(dialog);
            dialog.show();
            return _this.parentDialog.busy(true);
          };
        })(this));
      }
    };

    return EmbedTemplateFieldUI;
  ...
  ContentTools.EmbedDialog = (function(_super) {
    ...
    function EmbedDialog(origcaption, origcontent) {
      this.origcaption = origcaption;
      this.origcontent = origcontent;
      EmbedDialog.__super__.constructor.call(this, 'Embed HTML');
    }
    ...
    EmbedDialog.prototype.mount = function() {
      ...
      ContentEdit.addCSSClass(this._domView, 'ct-embed-dialog__view');
      this._domInputContent = document.createElement('textarea');
      this._domInputContent.setAttribute('class', 'ct-embed-dialog__textarea');
      this._domInputContent.setAttribute('name', 'content');
      this._domInputContent.setAttribute('placeholder', ContentEdit._('Paste embed code/HTML') + '...');
      this._domInputContent.value = this.origcontent;
      this._domView.appendChild(this._domInputContent);
      domControlGroup = this.constructor.createDiv(['ct-control-group', 'ct-control-group--left']);
      this._domControls.appendChild(domControlGroup);
      this._domInputCaption = document.createElement('input');
      this._domInputCaption.setAttribute('class', 'ct-embed-dialog__input');
      this._domInputCaption.setAttribute('name', 'caption');
      this._domInputCaption.setAttribute('placeholder', ContentEdit._('Caption') + '...');
      this._domInputCaption.setAttribute('type', 'text');
      this._domInputCaption.setAttribute('value', this.origcaption);
      domControlGroup.appendChild(this._domInputCaption);
      domControlGroup = this.constructor.createDiv(['ct-control-group', 'ct-control-group--right']);
      this._domControls.appendChild(domControlGroup);
      if (ContentTools.EMBED_TEMPLATES.length) {
        this._domTemplatesButton = this.constructor.createDiv(['ct-control', 'ct-control--text', 'ct-control--templates']);
        this._domTemplatesButton.textContent = ContentEdit._('Templates...');
        domControlGroup.appendChild(this._domTemplatesButton);
      } else {
        this._domTemplatesButton = null;
      }
      this._domEmbedButton = this.constructor.createDiv(['ct-control', 'ct-control--text', 'ct-control--insert']);
      if (!this.origcaption && !this.origcontent) {
        ContentEdit.addCSSClass(this._domEmbedButton, 'ct-control--muted');
      }
      this._domEmbedButton.textContent = ContentEdit._('Embed');
      domControlGroup.appendChild(this._domEmbedButton);
      if (ContentTools.EMBED_EDITOR) {
        ContentTools.EMBED_EDITOR.init({
          dialog: this,
          content: this._domInputContent
        });
      }
      return this._addDOMEventListeners();
    };
    ...
    EmbedDialog.prototype.save = function() {
      return this.dispatchEvent(this.createEvent('save', {
        'content': this._domInputContent.value.trim(),
        'caption': this._domInputCaption.value.trim()
      }));
    };

    EmbedDialog.prototype.show = function() {
      EmbedDialog.__super__.show.call(this);
      return this._domInputContent.focus();
    };

    EmbedDialog.prototype.unmount = function() {
      if (this.isMounted()) {
        this._domInputContent.blur();
        this._domInputCaption.blur();
      }
      EmbedDialog.__super__.unmount.call(this);
      this._domTemplatesButton = null;
      this._domEmbedButton = null;
      this._domInputCaption = null;
      return this._domInputContent = null;
    };

    EmbedDialog.prototype._addDOMEventListeners = function() {
      EmbedDialog.__super__._addDOMEventListeners.call(this);
      this._domInputContent.addEventListener('input', (function(_this) {
        return function(ev) {
          if (ev.target.value && _this._domInputCaption.value) {
            return ContentEdit.removeCSSClass(_this._domEmbedButton, 'ct-control--muted');
          } else {
            return ContentEdit.addCSSClass(_this._domEmbedButton, 'ct-control--muted');
          }
        };
      })(this));
      this._domInputCaption.addEventListener('input', (function(_this) {
        return function(ev) {
          if (ev.target.value && _this._domInputContent.value) {
            return ContentEdit.removeCSSClass(_this._domEmbedButton, 'ct-control--muted');
          } else {
            return ContentEdit.addCSSClass(_this._domEmbedButton, 'ct-control--muted');
          }
      ...
      this._domInputCaption.addEventListener('keypress', (function(_this) {
        return function(ev) {
          if (ev.keyCode === 13 && _this._domInputContent.value && _this._domInputCaption.value) {
            return _this.save();
          }
        };
      })(this));
      if (this._domTemplatesButton) {
        this._domTemplatesButton.addEventListener('click', (function(_this) {
          return function(ev) {
            var app, dialog;
            ev.preventDefault();
            app = ContentTools.EditorApp.get();
            ...
            dialog.addEventListener('save', function(ev) {
              ...
              _this._domInputContent.value = _this._domInputContent.value.trim() + '\n\n' + content;
              _this._domInputContent.value = _this._domInputContent.value.trim();
              if (_this._domInputContent.value && _this._domInputCaption.value) {
                ContentEdit.removeCSSClass(_this._domEmbedButton, 'ct-control--muted');
              } else {
                ContentEdit.addCSSClass(_this._domEmbedButton, 'ct-control--muted');
              }
              dialog.hide();
              return _this.busy(false);
            });
      ...
      return this._domEmbedButton.addEventListener('click', (function(_this) {
        return function(ev) {
          ev.preventDefault();
          if (_this._domInputContent.value && _this._domInputCaption.value) {
            return _this.save();
  ...
  ContentTools.Tools.Embed = (function(_super) {
    ...
    Embed.apply = function(element, selection, callback) {
      var app, caption, content, dialog, e, modal, toolDetail;
      toolDetail = {
        'tool': this,
        'element': element,
        'selection': selection
      ...
      dialog.addEventListener('save', (function(_this) {
        return function(ev) {
          var embed, index, node, previewcontent, _ref;
          ...
          if (element.type() === 'Embed') {
            ...
            embed = new ContentEdit.Embed('div-embed', {
              'aria-label': caption,
              'data-html': JSON.stringify(content)
            }, previewcontent);
            _ref = _this._insertAt(element), node = _ref[0], index = _ref[1];
  ...
  ContentTools.InsertFileDialog = (function(_super) {
    __extends(InsertFileDialog, _super);

    function InsertFileDialog() {
      InsertFileDialog.__super__.constructor.call(this, 'Insert File');
      this._runTimeout = null;
      this._items = [];
      this._uploads = 0;
      this._childModal = false;
    }

    InsertFileDialog.prototype.mount = function() {
      var domControlGroup, domSearchResult, domTabs, searchField, searchFields, searchFieldsWrap;
      InsertFileDialog.__super__.mount.call(this);
      ContentEdit.addCSSClass(this._domElement, 'ct-insert-file-dialog');
      ContentEdit.addCSSClass(this._domView, 'ct-insert-file-dialog__view');
      this._domSearch = this.constructor.createDiv(['ct-insert-file-dialog__search', 'ct-insert-file-dialog__search--active']);
      this._domView.appendChild(this._domSearch);
      searchFieldsWrap = this.constructor.createDiv(['ct-insert-file-dialog__search-fields-wrapper']);
      this._domSearch.appendChild(searchFieldsWrap);
      searchFields = this.constructor.createDiv(['ct-insert-file-dialog__search-fields']);
      searchFieldsWrap.appendChild(searchFields);
      searchField = this.constructor.createDiv(['ct-insert-file-dialog__search-field-left']);
      searchFields.appendChild(searchField);
      this._domSearchInput = document.createElement('input');
      this._domSearchInput.setAttribute('class', 'ct-insert-file-dialog__search-input');
      this._domSearchInput.setAttribute('name', 'search');
      this._domSearchInput.setAttribute('placeholder', ContentEdit._('Find') + '...');
      this._domSearchInput.setAttribute('type', 'text');
      searchField.appendChild(this._domSearchInput);
      searchField = this.constructor.createDiv(['ct-insert-file-dialog__search-field-right']);
      searchFields.appendChild(searchField);
      this._domSearchLimit = document.createElement('select');
      this._domSearchLimit.setAttribute('class', 'ct-insert-file-dialog__search-limit');
      this._domSearchLimit.setAttribute('name', 'limit');
      searchField.appendChild(this._domSearchLimit);
      this._domSearchResults = this.constructor.createDiv(['ct-insert-file-dialog__search-results']);
      this._domSearch.appendChild(this._domSearchResults);
      this._domUpload = this.constructor.createDiv(['ct-insert-file-dialog__upload']);
      this._domView.appendChild(this._domUpload);
      if (ContentTools.INSERT_FILE_UPLOADER) {
        ContentTools.INSERT_FILE_UPLOADER.init({
          dialog: this,
          upload: this._domUpload
        });
      } else {
        this._domUpload.innerHTML = ContentEdit._('ContentTools.INSERT_FILE_UPLOADER is not correctly configured.');
      }
      domTabs = this.constructor.createDiv(['ct-control-group', 'ct-control-group--left']);
      this._domControls.appendChild(domTabs);
      this._domSearchTab = this.constructor.createDiv(['ct-control', 'ct-control--icon', 'ct-control--icon-search', 'ct-control--active']);
      this._domSearchTab.setAttribute('data-ct-tooltip', ContentEdit._('Find/Select'));
      domTabs.appendChild(this._domSearchTab);
      if (!ContentTools.INSERT_FILE_FIND) {
        ContentEdit.addCSSClass(this._domSearchTab, 'ct-control--muted');
      }
      this._domUploadTab = this.constructor.createDiv(['ct-control', 'ct-control--icon', 'ct-control--icon-upload']);
      this._domUploadTab.setAttribute('data-ct-tooltip', ContentEdit._('Upload'));
      domTabs.appendChild(this._domUploadTab);
      if (!ContentTools.INSERT_FILE_UPLOADER) {
        ContentEdit.addCSSClass(this._domUploadTab, 'ct-control--muted');
      }
      domControlGroup = this.constructor.createDiv(['ct-control-group', 'ct-control-group--right']);
      this._domControls.appendChild(domControlGroup);
      this._domButton = this.constructor.createDiv(['ct-control', 'ct-control--text', 'ct-control--insert', 'ct-control--muted']);
      ...
      domControlGroup.appendChild(this._domButton);
      if (ContentTools.INSERT_FILE_FIND) {
        ContentTools.INSERT_FILE_FIND.init({
          dialog: this,
          searchinput: this._domSearchInput,
          searchlimit: this._domSearchLimit
        });
        ContentTools.INSERT_FILE_FIND.run();
      } else {
        domSearchResult = this.constructor.createDiv(['ct-insert-file-dialog__search-result']);
        domSearchResult.innerHTML = ContentEdit._('ContentTools.INSERT_FILE_FIND is not correctly configured.');
        this._domSearchResults.appendChild(domSearchResult);
      }
      return this._addDOMEventListeners();
    };

    InsertFileDialog.prototype.clearResults = function(all) {
      var item, items, titleUI, _i, _len, _ref;
      items = [];
      _ref = this._items;
      for (_i = 0, _len = _ref.length; _i < _len; _i++) {
        item = _ref[_i];
      ...
      if (items.length) {
        ...
        titleUI.mount(this._domSearchResults, items[0].domElement());
        items.unshift(titleUI);
      }
      this._items = items;
      return this._updateInsertButton();
    };

    InsertFileDialog.prototype.addResultError = function(message) {
      var titleUI;
      titleUI = new FindResultTitleUI(message, true);
      if (this._items.length) {
        titleUI.mount(this._domSearchResults, this._items[0].domElement());
      } else {
        titleUI.mount(this._domSearchResults);
      }
      return this._items.push(titleUI);
    };

    InsertFileDialog.prototype.addResultTitle = function(title) {
      var titleUI;
    ...
    InsertFileDialog.prototype.addResultItem = function(options, selected) {
      var itemUI;
      itemUI = new FindResultItemUI(this, options, selected);
      itemUI.mount(this._domSearchResults);
      this._items.push(itemUI);
      return this._updateInsertButton();
    };

    InsertFileDialog.prototype.uploadStarted = function() {
      this._uploads++;
      return this._updateInsertButton();
    };

    InsertFileDialog.prototype.uploadFinished = function() {
      this._uploads--;
      return this._updateInsertButton();
    };

    InsertFileDialog.prototype.childModal = function(display) {
      this._childModal = display;
      return this._updateInsertButton();
    };

    InsertFileDialog.prototype.selectedItems = function() {
    ...
    InsertFileDialog.prototype.save = function() {
      if (ContentTools.INSERT_FILE_FIND) {
        return this.dispatchEvent(this.createEvent('save', {
    ...
    InsertFileDialog.prototype.unmount = function() {
      if (this.isMounted()) {
        this._domSearchInput.blur();
        this._domSearchLimit.blur();
      }
      InsertFileDialog.__super__.unmount.call(this);
      this._domButton = null;
      this._domSearchLimit = null;
      return this._domSearchInput = null;
    };

    InsertFileDialog.prototype._updateInsertButton = function() {
      var item, _i, _len, _ref;
      if (this._childModal || this._uploads) {
        this.busy(true);
      } else {
        this.busy(false);
      }
    ...
    InsertFileDialog.prototype._findRunExecute = function() {
      this._runTimeout = null;
      return ContentTools.INSERT_FILE_FIND.run();
    };

    InsertFileDialog.prototype._findRun = function() {
      if (ContentTools.INSERT_FILE_FIND) {
        if (this._runTimeout) {
          clearTimeout(this._runTimeout);
        }
        return this._runTimeout = setTimeout((function(_this) {
          return function() {
            return _this._findRunExecute();
          };
        })(this), 250);
      }
    };

    InsertFileDialog.prototype._addDOMEventListeners = function() {
      InsertFileDialog.__super__._addDOMEventListeners.call(this);
      this._domSearchInput.addEventListener('input', (function(_this) {
        return function(ev) {
          return _this._findRun();
        };
      })(this));
      this._domSearchLimit.addEventListener('change', (function(_this) {
        return function(ev) {
          return _this._findRun();
        };
      })(this));
      this._domSearchLimit.addEventListener('keydown', (function(_this) {
        return function(ev) {
          return _this._findRun();
        };
      })(this));
      this._domSearchResults.addEventListener('click', (function(_this) {
        return function(ev) {
          return _this._updateInsertButton();
        };
      })(this));
      this._domSearchTab.addEventListener('mousedown', (function(_this) {
        return function() {
          ContentEdit.removeCSSClass(_this._domUpload, 'ct-insert-file-dialog__upload--active');
          ContentEdit.removeCSSClass(_this._domUploadTab, 'ct-control--active');
          ContentEdit.addCSSClass(_this._domSearch, 'ct-insert-file-dialog__search--active');
          return ContentEdit.addCSSClass(_this._domSearchTab, 'ct-control--active');
        };
      })(this));
      this._domUploadTab.addEventListener('mousedown', (function(_this) {
        return function() {
          ContentEdit.removeCSSClass(_this._domSearch, 'ct-insert-file-dialog__search--active');
          ContentEdit.removeCSSClass(_this._domSearchTab, 'ct-control--active');
          ContentEdit.addCSSClass(_this._domUpload, 'ct-insert-file-dialog__upload--active');
          return ContentEdit.addCSSClass(_this._domUploadTab, 'ct-control--active');
        };
      })(this));
      return this._domButton.addEventListener('click', (function(_this) {
        return function(ev) {
          ev.preventDefault();
          if (!_this.busy() && _this._domButton.getAttribute('class').indexOf('ct-control--muted') === -1) {
            return _this.save();
          }
        };
      })(this));
    ...
    return InsertFileDialog;

  })(ContentTools.DialogUI);

  FindResultTitleUI = (function(_super) {
    __extends(FindResultTitleUI, _super);

    function FindResultTitleUI(title, error) {
      this.title = title;
      this.error = error != null ? error : false;
      FindResultTitleUI.__super__.constructor.call(this);
    }

    FindResultTitleUI.prototype.mount = function(domParent, before) {
      var label, section;
      if (before == null) {
        before = null;
      }
      this._domElement = this.constructor.createDiv(['ct-section-wrap']);
      section = this.constructor.createDiv(['ct-section']);
      if (this.error) {
        ContentEdit.addCSSClass(section, 'ct-section--error');
      } else {
        ContentEdit.addCSSClass(section, 'ct-section--title');
      }
      this._domElement.appendChild(section);
      label = this.constructor.createDiv(['ct-section__label']);
      label.textContent = this.title;
      section.appendChild(label);
      return FindResultTitleUI.__super__.mount.call(this, domParent, before);
    };

    return FindResultTitleUI;

  })(ContentTools.AnchoredComponentUI);

  FindResultItemUI = (function(_super) {
    __extends(FindResultItemUI, _super);

    function FindResultItemUI(parentDialog, options, selected) {
      this.parentDialog = parentDialog;
      this.options = options;
      FindResultItemUI.__super__.constructor.call(this);
      this._selected = selected;
      this._mode = 0;
    }

    FindResultItemUI.prototype.selected = function(selected) {
      if (selected === void 0) {
      ...
      this._selected = selected;
      if (this._selected) {
        return ContentEdit.addCSSClass(this._section, 'ct-section--applied');
      } else {
        return ContentEdit.removeCSSClass(this._section, 'ct-section--applied');
      }
    };

    FindResultItemUI.prototype.mode = function() {
      this.options.modes[this._mode].label = this.options.label;
      return this.options.modes[this._mode];
    };

    FindResultItemUI.prototype.mount = function(domParent, before) {
      if (before == null) {
        before = null;
      }
      this._domElement = this.constructor.createDiv(['ct-section-wrap']);
      this._section = this.constructor.createDiv(['ct-section']);
      if (this._selected) {
        ContentEdit.addCSSClass(this._section, 'ct-section--applied');
      }
      this._domElement.appendChild(this._section);
      this._domCheckbox = this.constructor.createDiv(['ct-section__icon', 'ct-section__checkbox']);
      this._section.appendChild(this._domCheckbox);
      this._domLabel = this.constructor.createDiv(['ct-section__label']);
      this._domLabel.textContent = this.options.label;
      this._section.appendChild(this._domLabel);
      if (this.options.preview) {
        this._domPreviewWrap = this.constructor.createDiv(['ct-section__icon', 'ct-section__preview']);
      } else {
        this._domPreviewWrap = this.constructor.createDiv(['ct-section__icon', 'ct-section__new-tab']);
      }
      this._section.appendChild(this._domPreviewWrap);
      this._domModeWrap = this.constructor.createDiv(['ct-section__icon', 'ct-section__mode-' + this.options.modes[this._mode].icon]);
      this._domModeWrap.title = this.options.modes[this._mode].title;
      this._section.appendChild(this._domModeWrap);
      return FindResultItemUI.__super__.mount.call(this, domParent, before);
    };

    FindResultItemUI.prototype.unmount = function() {
      FindResultItemUI.__super__.unmount.call(this);
      return this.parentDialog = null;
    };

    FindResultItemUI.prototype._addDOMEventListeners = function() {
      this._domCheckbox.addEventListener('click', (function(_this) {
        return function(ev) {
          ev.preventDefault();
          if (_this._selected) {
      ...
      this._domLabel.addEventListener('click', (function(_this) {
        return function(ev) {
          ev.preventDefault();
          if (_this._selected) {
            return _this.selected(false);
      ...
      if (this.options.preview) {
        this._domPreviewWrap.addEventListener('click', (function(_this) {
          return function(ev) {
            var app, modal;
            ev.preventDefault();
            app = ContentTools.EditorApp.get();
            modal = new ContentTools.PreviewModalUI(_this.options.preview());
            modal.addEventListener('cancel', function(ev) {
              modal.hide();
              return _this.parentDialog.childModal(false);
            });
            app.attach(modal);
            modal.show();
            return _this.parentDialog.childModal(true);
          };
        })(this));
      } else {
        this._domPreviewWrap.addEventListener('click', (function(_this) {
          return function(ev) {
            ev.preventDefault();
            return _this.options.link.click();
          };
        })(this));
      }
      return this._domModeWrap.addEventListener('click', (function(_this) {
        return function(ev) {
          ContentEdit.removeCSSClass(_this._domModeWrap, 'ct-section__mode-' + _this.options.modes[_this._mode].icon);
          _this._mode++;
          if (_this._mode >= _this.options.modes.length) {
            _this._mode = 0;
          }
          ContentEdit.addCSSClass(_this._domModeWrap, 'ct-section__mode-' + _this.options.modes[_this._mode].icon);
          return _this._domModeWrap.title = _this.options.modes[_this._mode].title;
        };
      })(this));
    };

    return FindResultItemUI;

  })(ContentTools.AnchoredComponentUI);

  ContentTools.PreviewModalUI = (function(_super) {
    __extends(PreviewModalUI, _super);

    function PreviewModalUI(preview) {
      this.preview = preview;
      this._handleEscape = __bind(this._handleEscape, this);
      PreviewModalUI.__super__.constructor.call(this);
    }

    PreviewModalUI.prototype.mount = function() {
      var app, innerElement;
      this._domElement = this.constructor.createDiv(['ct-widget', 'ct-modal', 'ct-preview-modal']);
      this.parent().domElement().appendChild(this._domElement);
      innerElement = this.constructor.createDiv(['ct-preview-modal-inner']);
      this._domElement.appendChild(innerElement);
      this._domPreview = this.preview.cloneNode(true);
      innerElement.appendChild(this._domPreview);
      app = ContentTools.EditorApp.get();
      app.addedModal();
      return this._addDOMEventListeners();
    };

    PreviewModalUI.prototype.unmount = function() {
      var app;
      app = ContentTools.EditorApp.get();
      app.removedModal();
      return PreviewModalUI.__super__.unmount.call(this);
    };

    PreviewModalUI.prototype._handleEscape = function(ev) {
      if (ev.keyCode === 27) {
        return this.dispatchEvent(this.createEvent('cancel'));
      }
    };

    PreviewModalUI.prototype._addDOMEventListeners = function() {
      document.addEventListener('keyup', this._handleEscape);
      return this._domElement.addEventListener('click', (function(_this) {
        return function(ev) {
          return _this.dispatchEvent(_this.createEvent('cancel'));
    ...
    PreviewModalUI.prototype._removeDOMEventListeners = function() {
      return document.removeEventListener('keyup', this._handleEscape);
    };

    return PreviewModalUI;

  })(ContentTools.WidgetUI);

  ContentTools.Tools.InsertFile = (function(_super) {
    __extends(InsertFile, _super);

    function InsertFile() {
      return InsertFile.__super__.constructor.apply(this, arguments);
    }

    ContentTools.ToolShelf.stow(InsertFile, 'insert-file');

    InsertFile.label = 'Insert file';

    InsertFile.icon = 'insert-file';

    InsertFile.canApply = function(element, selection) {
      return !element.isFixed();
    };

    InsertFile.apply = function(element, selection, callback) {
      var app, dialog, modal, startpos, toolDetail;
      toolDetail = {
        'tool': this,
        'element': element,
        'selection': selection
      ...
      if (element.content) {
        while (startpos < element.content.characters.length && element.content.characters[startpos].hasTags('a')) {
          startpos++;
        }
      }
      if (element.storeState) {
      ...
      dialog.addEventListener('save', (function(_this) {
        return function(ev) {
          var a, embed, haslink, img, index, item, linkopts, node, selected, text, _i, _len, _ref, _ref1, _ref2;
          selected = ev.detail().selected;
          haslink = false;
          for (_i = 0, _len = selected.length; _i < _len; _i++) {
            item = selected[_i];
            if (item.element === 'img') {
              img = new ContentEdit.Image({
                'src': item.url,
                'width': item.width,
                'height': item.height,
                'data-src-info': JSON.stringify(item.info)
              });
              _ref = _this._insertAt(element), node = _ref[0], index = _ref[1];
              ...
              linkopts = {
                'href': item.url,
                'data-src-info': HTMLString.String.encode(JSON.stringify(item.info)).replace(/"/g, '&quot;')
              };
              if (item.target) {
              ...
              _ref2 = _this._insertAt(element), node = _ref2[0], index = _ref2[1];
              node.parent().attach(embed, index);
              embed.focus();
            }
          }
  ...
  ContentTools.CropImageDialog = (function(_super) {
    __extends(CropImageDialog, _super);

    function CropImageDialog(element) {
      this.element = element;
      this._handleWindowResize = __bind(this._handleWindowResize, this);
      CropImageDialog.__super__.constructor.call(this, 'Crop Image');
    ...
    CropImageDialog.prototype.mount = function() {
      var domControlGroup, domViewInfo, e;
      CropImageDialog.__super__.mount.call(this);
      ContentEdit.addCSSClass(this._domElement, 'ct-crop-image-dialog');
      ContentEdit.addCSSClass(this._domView, 'ct-crop-image-dialog__view');
      domControlGroup = this.constructor.createDiv(['ct-control-group', 'ct-control-group--right']);
      this._domControls.appendChild(domControlGroup);
      if (ContentTools.CROP_IMAGE) {
        this._domRatios = this.constructor.createDiv(['ct-crop-image-dialog__ratios']);
        this._domView.appendChild(this._domRatios);
        this._domCrop = this.constructor.createDiv(['ct-crop-image-dialog__crop']);
        this._domView.appendChild(this._domCrop);
        if (ContentTools.CROP_IMAGE.setdefault) {
          this._domSetDefaultButton = this.constructor.createDiv(['ct-control', 'ct-control--text', 'ct-control--muted']);
          this._domSetDefaultButton.textContent = ContentEdit._('Set default');
          domControlGroup.appendChild(this._domSetDefaultButton);
        }
      } else {
        domViewInfo = this.constructor.createDiv(['ct-crop-image-dialog__view-info']);
        domViewInfo.innerHTML = ContentEdit._('ContentTools.CROP_IMAGE is not correctly configured.');
        this._domView.appendChild(domViewInfo);
      }
      this._domApplyButton = this.constructor.createDiv(['ct-control', 'ct-control--text', 'ct-control--apply', 'ct-control--muted']);
      this._domApplyButton.textContent = ContentEdit._('Apply');
      domControlGroup.appendChild(this._domApplyButton);
      if (ContentTools.CROP_IMAGE) {
        try {
          ContentTools.CROP_IMAGE.init({
            dialog: this,
            crop: this._domCrop,
            info: JSON.parse(this.element.attr('data-src-info'))
          });
        } catch (_error) {
          e = _error;
        }
    ...
    CropImageDialog.prototype.addCropRatio = function(ratio, display, info) {
      var itemUI;
      itemUI = new CropRatioItemUI(this, ratio, display, info);
      itemUI.mount(this._domRatios);
      return this._items.push(itemUI);
    };

    CropImageDialog.prototype.selectCropRatio = function(newratio) {
      ...
      if (this._selected < 0) {
        ContentEdit.addCSSClass(this._domApplyButton, 'ct-control--muted');
        if (ContentTools.CROP_IMAGE.setdefault) {
          return ContentEdit.addCSSClass(this._domSetDefaultButton, 'ct-control--muted');
        }
      } else {
        ContentTools.CROP_IMAGE.ratio(newratio, this._items[this._selected].info());
        ContentEdit.removeCSSClass(this._domApplyButton, 'ct-control--muted');
        if (ContentTools.CROP_IMAGE.setdefault) {
          return ContentEdit.removeCSSClass(this._domSetDefaultButton, 'ct-control--muted');
        }
      }
    };

    CropImageDialog.prototype.updateCropRatioInfo = function(newinfo) {
      return this._items[this._selected].info(newinfo);
    };

    CropImageDialog.prototype.selectedCropRatio = function() {
      return this._items[this._selected].ratio();
    };

    CropImageDialog.prototype.getCropRatioInfo = function() {
      return this._items[this._selected].info();
    };

    CropImageDialog.prototype.save = function() {
      var crops, item, result, _i, _len, _ref;
      crops = {};
      _ref = this._items;
      for (_i = 0, _len = _ref.length; _i < _len; _i++) {
        item = _ref[_i];
      ...
      result = ContentTools.CROP_IMAGE.save(this._items[this._selected].ratio(), crops);
      return this.dispatchEvent(this.createEvent('save', result));
    };

    CropImageDialog.prototype._handleWindowResizeFinal = function() {
      this._resizeTimeout = null;
      return ContentTools.CROP_IMAGE.resize();
    };

    CropImageDialog.prototype._handleWindowResize = function(ev) {
      if (ContentTools.CROP_IMAGE && ContentTools.CROP_IMAGE.resize) {
        if (this._resizeTimeout) {
          clearTimeout(this._resizeTimeout);
        }
        return this._resizeTimeout = setTimeout((function(_this) {
          return function() {
            return _this._handleWindowResizeFinal();
          };
        })(this), 250);
      }
    };
    ...
    CropImageDialog.prototype._addDOMEventListeners = function() {
      ...
      if (ContentTools.CROP_IMAGE && ContentTools.CROP_IMAGE.setdefault) {
        this._domSetDefaultButton.addEventListener('click', (function(_this) {
          return function(ev) {
            ev.preventDefault();
            if (_this._selected > -1) {
              return ContentTools.CROP_IMAGE.setdefault(_this._items[_this._selected].ratio(), _this._items[_this._selected].info());
            }
          };
        })(this));
      }
      this._domApplyButton.addEventListener('click', (function(_this) {
        return function(ev) {
          ev.preventDefault();
          if (_this._selected > -1) {
            return _this.save();
      ...
      return window.addEventListener('resize', this._handleWindowResize);
    };

    CropImageDialog.prototype._removeDOMEventListeners = function() {
      return window.removeEventListener('resize', this._handleWindowResize);
    };
  ...
  CropRatioItemUI = (function(_super) {
    __extends(CropRatioItemUI, _super);

    function CropRatioItemUI(parentDialog, ratio, display, info) {
      this.parentDialog = parentDialog;
      this.display = display;
      CropRatioItemUI.__super__.constructor.call(this);
      this._selected = false;
      this._ratio = ratio;
      this._info = info;
    }

    CropRatioItemUI.prototype.selected = function(selected) {
      if (selected === void 0) {
        return this._selected;
      }
      if (this._selected === selected) {
    ...
    CropRatioItemUI.prototype.ratio = function() {
      return this._ratio;
    };

    CropRatioItemUI.prototype.info = function(newinfo) {
      if (newinfo === void 0) {
        return this._info;
      }
      return this._info = newinfo;
    };

    CropRatioItemUI.prototype.mount = function(domParent, before) {
      if (before == null) {
        before = null;
      }
      this._domElement = this.constructor.createDiv(['ct-section-wrap']);
      this._section = this.constructor.createDiv(['ct-section']);
      this._domElement.appendChild(this._section);
      this._domRadio = this.constructor.createDiv(['ct-section__icon', 'ct-section__radio']);
      this._section.appendChild(this._domRadio);
      this._domLabel = this.constructor.createDiv(['ct-section__label']);
      this._domLabel.textContent = this.display;
      this._section.appendChild(this._domLabel);
      return CropRatioItemUI.__super__.mount.call(this, domParent, before);
    };

    CropRatioItemUI.prototype.unmount = function() {
      CropRatioItemUI.__super__.unmount.call(this);
      return this.parentDialog = null;
    };

    CropRatioItemUI.prototype._addDOMEventListeners = function() {
      return this._section.addEventListener('click', (function(_this) {
        return function(ev) {
          ev.preventDefault();
          if (!_this._selected) {
            return _this.parentDialog.selectCropRatio(_this._ratio);
          }
        };
      })(this));
    };

    return CropRatioItemUI;

  })(ContentTools.AnchoredComponentUI);

  ContentTools.Tools.CropImage = (function(_super) {
    ...
    ContentTools.ToolShelf.stow(CropImage, 'crop-image');

    CropImage.label = 'Crop image';

    CropImage.icon = 'crop-image';

    CropImage.canApply = function(element, selection) {
      return element.type() === 'Image' && element.attr('data-src-info') && !element.isFixed();
    };

    CropImage.apply = function(element, selection, callback) {
      var app, dialog, modal, toolDetail;
      toolDetail = {
        'tool': this,
        'element': element,
      ...
      dialog.addEventListener('save', (function(_this) {
        return function(ev) {
          var attributes, details, img, index, key, node, val, _ref, _ref1;
          details = ev.detail();
          attributes = {};
          _ref = element._attributes;
          for (key in _ref) {
            val = _ref[key];
            attributes[key] = val;
          }
  ...
  ContentTools.DEFAULT_TOOLS = [['bold', 'italic', 'link', 'align-left', 'align-center', 'align-right'], ['paragraph', 'h2', 'h3', 'h4', 'h5', 'h6', 'blockquote', 'preformatted', 'line-break', 'unordered-list', 'ordered-list', 'table', 'indent', 'unindent', 'embed'], ['insert-file', 'crop-image'], ['undo', 'redo', 'remove']];
